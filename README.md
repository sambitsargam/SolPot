...
A multi-game on-chain arena on Solana featuring word guessing, lucky number picks, and trivia challenges — with Jupiter swap entry, Arcium-style encrypted guesses, Metaplex NFT rewards, Magicblock-pattern real-time updates, and dark/light mode.

## Features

- **Multi-Game Portal** — Three game modes powered by a single Anchor smart contract: Word Guess, Lucky Number (1–100 grid), and Trivia Challenge
...
> All three modes use the **same deployed Solana program** — the contract stores SHA-256 hashes and compares text guesses. The game type only changes the frontend input UI.
...
3. **Players connect wallet** and choose a game mode from the Portal Hub
4. **Players enter** by paying the entry fee (SOL or any SPL token via Jupiter swap)
5. **Players submit guesses** — word input, number grid click, or trivia option selection
6. **First correct guess wins** — round closes, winner is recorded
